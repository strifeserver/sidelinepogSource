...
class BetController extends Controller
{
    public function placeBet(Request $request){
        ...
        //return response()->json(['msg'=>'Internal Server Error'],200);
        if(Auth::user()->type == 'player'){
            ...
            $up = getUplineCommission(Auth::id());
            ...
            if($wallet->balance < $request['amount']){
                ...
                $myBetMeron = Bet::where('fight_id',$request['fight_id'])->where('user_id',Auth::id())->where('bet','meron')->sum('amount');
                $myBetWala = Bet::where('fight_id',$request['fight_id'])->where('user_id',Auth::id())->where('bet','wala')->sum('amount');
                $myBetDraw = Bet::where('fight_id',$request['fight_id'])->where('user_id',Auth::id())->where('bet','draw')->sum('amount');
                ...
                $betData = [
                    'betMeron' => bcdiv($betMeron+$fight->ghost_bet_meron,1,2),
                    'betWala' => bcdiv($betWala+$fight->ghost_bet_wala,1,2),
