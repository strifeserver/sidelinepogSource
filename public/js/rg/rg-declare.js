...
$('.btn-choose3').on('click',function(){
    ...
    $('#selected-third').css('background-color',color);
});

$('.btn-choose2').on('click',function(){
    let color = $(this).data('color')
    $('#second-color').val(color);
    $('#selected-second').css('background-color',color);
});

$('.btn-choose1').on('click',function(){
    let color = $(this).data('color')
    $('#first-color').val(color);
    $('#selected-first').css('background-color',color);
});
...
$('.btn-ghost-bet').on('click',function(){
    ...
    $.ajax({
        ...
        data : data,
        success:function(res){
            updateBets(res.betOne,res.betTwo,res.betFive,res.betTen,res.betTwenty,res.betForty);
        },
        error: function(err){
            console.log(err);
        }
